...
- [x] Use document scroll (`html/body`) for full-page rendering and screenshot compatibility.
- [x] Keep modal/body locking behavior safe so overlays still trap focus and scroll as expected.
- [x] Keep sticky header/rails working with document scroll and responsive breakpoints.
- [x] Verify Firefox full-page screenshots capture full content height.
...
- [x] Remove wishlist shelf from main run-tracking library dashboard.
- [x] Keep primary library emphasis on:
  - playing
  - finished
  - backlog
- [x] Ensure backlog/wishlist continue hiding run-only stats when not applicable.
...
- [x] Move add flow entrypoint emphasis from Library into Discover.
- [x] Introduce discover-first search results pattern:
  - type query
  - choose game
  - open log modal/workspace
- [x] Keep manual entry fallback for unmatched titles.
